...
export default function Sidebar({ isCollapsed }) {
  ...
  const sidebarWidth = isCollapsed ? '75px' : '220px';
  const collapsedNavHeight = options.length * 75 + 16;

  const sidebarStyle = {
    width: sidebarWidth,
    backgroundColor: isCollapsed ? '#FFFFFF' : '#EDF0F4', // Default collapsed = white
    minHeight: isCollapsed ? `${collapsedNavHeight}px` : '100vh',
      ...
      <div style={navContainerStyle}>
        {options.map(opt => {
          ...
          const itemStyle = {
            ...
            // Active item is blue; others transparent
